...
impl ClaudeConfig {
    ...
    pub fn write(&self) -> Result<(), String> {
        ...
        let obj = value.as_object_mut().ok_or("Settings file is not a JSON object")?;

        // Update only the env section
        obj.insert("env".to_string(), serde_json::to_value(&self.env).unwrap());
    ...
    pub fn apply_provider(&self, provider: &ProviderEnvConfig) -> Result<(), String> {
        let mut config = Self::read()?;

        config.env.insert("ANTHROPIC_AUTH_TOKEN".to_string(), provider.api_key.clone());
        config.env.insert("ANTHROPIC_BASE_URL".to_string(), provider.base_url.clone());
        config.env.insert("ANTHROPIC_MODEL".to_string(), provider.model.clone());
        config.env.insert(
    ...
    /// Clear all ANTHROPIC env vars from the config
    pub fn clear_provider() -> Result<(), String> {
        let mut config = Self::read()?;
